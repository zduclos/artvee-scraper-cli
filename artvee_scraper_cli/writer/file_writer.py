...
class JsonFileWriter(AbstractWriter):
    ...
    def write(self, artwork: Artwork) -> bool:
        ...
        """
        try:
            file_path = f"{self._dir_path}{os.path.sep}{artwork.resource}.json"
...
class WriteImageCommand(Command):
    ...
    def __init__(
        ...
        self._image = artwork.image.raw

        self._path = f"{dir_path}{os.path.sep}{artwork.resource}.jpg"
...
class WriteMetadataCommand(Command):
    ...
    def __init__(
        ...
        self._path = f"{dir_path}{os.path.sep}{artwork.resource}.json"
    ...
    def execute(self) -> bool:
        ...
        try:
            with open(self._path, self._open_option, encoding="UTF-8") as fout:
                artwork_dict = self._artwork.to_dict()

                if image_dict := artwork_dict.get("image"):
                    image_dict.pop("raw")
